...
def SNPconv_layer(in_dim, out_dim, kernel_size=1, padding=0, stride=1):
    return nn.Sequential(
                        nn.BatchNorm2d(in_dim), nn.ReLU(True),
                        nn.Conv2d(in_dim, out_dim, kernel_size, stride, padding, bias=False))
...
def SNPdeconv_layer(in_dim, out_dim, kernel_size=1, padding=0, stride=1):
        return nn.Sequential(
                             nn.BatchNorm2d(in_dim), nn.ReLU(True),
                             nn.ConvTranspose2d(in_dim, out_dim, kernel_size, stride, padding, bias=False))

def linear_layer(in_dim, out_dim, bias=False):
    return nn.Sequential(nn.BatchNorm1d(in_dim), nn.ReLU(True),
...
class GALayer(nn.Module):
    def __init__(self,
        ...
        #改
        #self.ffn = nn.Sequential(nn.ReLU(True),nn.Dropout(dropout),
